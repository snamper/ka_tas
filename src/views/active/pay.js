...
var vm=new Moon({
	...
	data:{
		off:{
			load:0,//0,初始状态;1,正在支付;2,获取支付结果中;3,支付成功，待提交申请;4,提交申请中;
			payType:"0",//0,未选择支付方式;1,--;2,微信;3,支付宝;
			errorStatus:0,//0,无错误;1,支付失败;2,订单关闭;3,提交开卡失败;
		},
		orderInfo: {
	...
	methods:{
		payComplete:function(status){//支付完成
			...
			if(status==1){//支付成功
				...
				layer.open({
					title:'支付取消',
					content:'您已取消支付，如果想重新发起支付，请重新选择支付方式',
					btn:['确定'],
					shadeClose:false,
				});
			}else if(status==-1){
